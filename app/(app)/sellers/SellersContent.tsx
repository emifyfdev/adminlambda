...
import { createSeller, updateSeller, deleteSeller } from "@/lib/repos/sellers-repo"
...
export default function SellersContent({ sellersIniciales }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return sellersIniciales.filter((v) => {
      const hay = `${v.name ?? ""} ${v.email ?? ""} ${v.phone ?? ""} ${v.sales_team ?? ""} ${v.status ?? ""}`.toLowerCase()
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              <tbody>
                {filtered.map((s) => (
                  <tr key={s.id} className="border-t">
                    ...
                    <td className="p-3">{s.sales_team ?? "-"}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="destructive" size="sm" onClick={() => setConfirmDeleteId(s.id)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{mode === "create" ? "Nuevo vendedor" : "Editar vendedor"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, sales_team: v as SalesTeam })}
              >
                <SelectTrigger>
            ...
            <div className="grid gap-2">
              ...
              <Select
                value={form.status}
                onValueChange={(v) => setForm({ ...form, status: v as SellerStatus })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Seleccionar..." />
                </SelectTrigger>
